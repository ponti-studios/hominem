...
States:
- Hover: CSS transition background-color 120ms ease
...
| Transition | border-color 120ms ease (CSS only) |

States:
- Default: border-default
- Focused: border upgrades to 2px accent
- Error: destructive border, error message in body-4/destructive beneath field
...
- Success state: do not add a green checkmark unless the field has async validation (e.g. username availability)
...
Interactive cards (clickable) get cursor: pointer and the pressed GSAP scale treatment.
...
Dismiss: backdrop click, Escape key, or explicit close button.
...
Same focus rules as Sheet.
Only one modal open at a time. Never nest modals.
...
Rules:
- Never use a sad face or error imagery for an empty state — it's not an error
