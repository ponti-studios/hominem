 ...
 *   npx @hackefeller/type-audit dashboard --audit-first [--threshold 1.0]
 * 
 * Examples:
 *   # Generate from existing audit
 *   npx @hackefeller/type-audit dashboard --input .type-analysis/report.json
 * 
 *   # Run audit first, then generate
 *   npx @hackefeller/type-audit dashboard --audit-first --threshold 0.5
 * 
 *   # Open in browser automatically
 *   npx @hackefeller/type-audit dashboard --input report.json --open
 */
...
function loadDashboardTemplate(): string {
  const templatePath = join(MODULE_DIR, 'templates', 'type-dashboard.html');
...
function runAudit(threshold: number): string {
  ...
  try {
    execSync(
      `node "${join(MODULE_DIR, 'index.js')}" audit --json "${outputFile}" --threshold ${threshold}`,
      {
        cwd: ROOT,
        stdio: 'inherit',
      }
