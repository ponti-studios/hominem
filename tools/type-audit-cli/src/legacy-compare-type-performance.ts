...
function parseArgs(): { baseline: string; current: string } {
  ...
  if (!baseline || !current) {
    console.error('Usage: type-audit compare --baseline <file> --current <file>');
    process.exit(1);
  }
...
function main() {
  ...
  const sortedChanges = changes.sort((a, b) => {
    const statusOrder: Record<'critical' | 'warning' | 'good', number> = {
      critical: 0,
      warning: 1,
      good: 2
    };
    const aStatus: 'critical' | 'warning' | 'good' =
      a.change.status === 'critical' || a.change.status === 'warning' ? a.change.status : 'good';
    const bStatus: 'critical' | 'warning' | 'good' =
      b.change.status === 'critical' || b.change.status === 'warning' ? b.change.status : 'good';
