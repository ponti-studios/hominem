# Career and public portfolio

## Purpose

Represent career history, job-search activity, and a public-facing professional profile (`apps/career`). This chapter did not exist in the original design; it is added during reconciliation because production implements a substantial, undocumented domain here — including the platform's only intentionally **public** data.
...
This chapter has **two separate sensitivity tiers**, unlike every other chapter in this Bible:

- `app.portfolios` and its published sub-entities are **intentionally public** once `is_public`/`is_visible` is set — this is the one deliberate exception to [00-platform-principles.md](00-platform-principles.md)'s "private by default."
...
None. Whether compensation fields should be split into a separate, more restrictively-scoped table (rather than living on the same row as publishable fields) is worth a follow-up decision but is out of scope for this reconciliation pass.
