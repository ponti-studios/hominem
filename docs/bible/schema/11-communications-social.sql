-- Reconciled Up: sender-only resolution, no participant roster, no message attachments, no social_posts.
...
CREATE TABLE app.communication_threads (
  ...
  external_id text, title text, channel text NOT NULL CHECK (channel IN ('email','sms','signal','social','other')),
...
CREATE TABLE app.communication_messages (
  ...
  body_artifact_id uuid REFERENCES app.import_artifacts(id) ON DELETE SET NULL, metadata jsonb NOT NULL DEFAULT '{}',
