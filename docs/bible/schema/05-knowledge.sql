-- Reconciled Up: notes double as versioned CMS content; no separate documents table.
...
CREATE TABLE app.extracted_facts (
  ...
  source_record_id uuid REFERENCES app.import_records(id) ON DELETE SET NULL, confidence numeric(5,4) NOT NULL DEFAULT 1 CHECK (confidence BETWEEN 0 AND 1),
...
-- note_files is defined in 02-provenance-files.sql. note_shares is a second, note-scoped sharing mechanism (see chapter divergence note).
