-- Reconciled Up, executed last: production's actual RLS shape is richer than a single owner-scoped policy per table.
...
--     not left entirely to the application -- narrower than 18-career-portfolio.md's privacy note originally implied.
...
--   auth.owns_import_source(source_id), auth.can_access_entity(entity_table, entity_id) -- import provenance and the entities registry
--
-- Example (app.notes):
...
BEGIN
  ...
  LOOP
    ...
    -- A table needing wider read access (spaces, notes, tags, portfolios, import provenance, the entities registry)
...
-- by application code -- see 00-foundation.sql. Its own RLS read policy uses auth.can_access_entity(entity_table, entity_id).

-- ops.audit_logs / ops.search_logs use a distinct service-role-only policy (see 01-identity-access.sql) --
