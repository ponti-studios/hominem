-- Reconciled Up: real finance_* naming; amount lives on both the transaction and its postings (see decision note).
...
-- amount is intentionally present on both the transaction and its postings; see 04-finance.md divergence note.
...
CREATE TABLE app.finance_statement_periods (
  ...
  starts_on date NOT NULL, ends_on date NOT NULL, opening_balance numeric(14,2), closing_balance numeric(14,2), statement_artifact_id uuid REFERENCES app.import_artifacts(id) ON DELETE SET NULL,
