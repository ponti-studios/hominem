# Mobile Readiness Tasks Mirror

## Mirror Metadata
- source: `/Users/charlesponti/Developer/hominem/specs/001-remove-any-types/tasks.md`
- branch: `001-fix-mobile-app-build-maestro`
- workflow: simulated `workflows:create` via `.specify` task template and strict checklist generation
- mirrored_at: `2026-02-25T23:43:00-08:00`

## Traceability Map (Plan Phase -> Task IDs)
- Phase 1 Workflow Artifact Safety + Planning Baseline -> T001-T005
- Phase 2 Mobile Auth Runtime Hardening Foundation -> T006-T012
- Phase 3 Deterministic Mobile Auth Bootstrap (US1) -> T013-T022
- Phase 4 Maestro Reliability (US2) -> T023-T031
- Phase 5 Personal Device Dev Build Lane (US3) -> T032-T038
- Phase 6 Polish + Cross-Cutting Gates -> T039-T042

## Execution Order Summary
- Critical path: T001 -> T003 -> T006 -> T016 -> T026 -> T034 -> T041 -> T042
- Parallel batches:
  - Batch A: T004 + T005
  - Batch B: T007 + T008 + T010
  - Batch C: T013 + T014
  - Batch D: T028 + T029 + T030
  - Batch E: T032 + T033 + T037 + T038
- Gate checks:
  - Callback routing and auth contract checks pass (`test:e2e:auth:api`, `test:e2e:auth:live`)
  - Maestro smoke/auth flows parse and execute without schema errors
  - EAS dev build install path documented and reproducible

## Auth Suite Run Log (T041)
- `2026-02-25`: `bun run test:e2e:auth:api` -> PASS (4 files, 14 tests)
- `2026-02-25`: `bun run test:e2e:auth:live` -> FAIL (`https://auth.ponti.io/api/status` returned `502`)
- `2026-02-25`: `bun test apps/mobile/utils/auth-provider.test.ts apps/mobile/utils/better-auth-mobile.test.ts` -> PASS (6 tests)
- `2026-02-25`: `bun run test:e2e:auth:live` -> FAIL with detailed edge telemetry (`server=cloudflare`, `cf-ray=9d35d8e99eb4f8b7-LAX`, upstream `502` at `/`)
