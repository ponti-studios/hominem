-- Up: events play the "series" role; recurrence is jsonb, not an RRULE string.
CREATE TABLE app.calendars (
  ...
  source text, external_id text, name text NOT NULL CHECK (btrim(name) <> ''), color text, timezone text, is_primary boolean NOT NULL DEFAULT false,
