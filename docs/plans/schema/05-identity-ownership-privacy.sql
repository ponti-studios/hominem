-- Up: sharing/spaces, and the real (service-role) audit mechanism. Better Auth owns authentication tables.
...
-- Audit is service-role scoped (auth.is_service_role()), not owner-scoped: excluded from row-level-security.sql's owner_isolation loop.
