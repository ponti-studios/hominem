...
The current interaction surface is also fragmented. The header menu, transform affordance row, and classification review sheet use different spacing, button treatment, and dismissal behavior. That makes the UI harder to reason about and harder to evolve safely. This change should consolidate those surfaces into one design-system-backed actions model with consistent tap targets, destructive handling, and state transitions.
...
- Redesign the actions interface so it matches the design system and uses a single shared interaction model.
...
### Build the actions UI from shared design-system primitives

The mobile actions surface should use the same visual language as the rest of the app: compact rows, clear hierarchy, consistent destructive styling, and predictable dismissal behavior. The implementation should prefer a single shared component for the conversation actions entry point instead of maintaining separate visual treatment for header actions, transform shortcuts, and review controls.

Alternatives considered:
- Keep the current ad hoc button/menu mix and only restyle it. Rejected because it still leaves the interaction model fragmented.
- Introduce a heavy custom gesture system. Rejected because the existing design-system primitives already cover the required states.
...
### Separate command selection from side-effect execution

The actions UI should only choose intent. Saving, previewing, destructive archive behavior, and source resolution should remain in controller and lifecycle code. This keeps the view layer predictable and makes it easier to evolve the action sheet without coupling it to persistence details.

Alternatives considered:
- Let the sheet call persistence directly. Rejected because it would spread backend knowledge into presentation code.
...
- [Interaction drift] If the header menu, transform row, and review sheet diverge again, users will see inconsistent control patterns. → Route all three surfaces through shared action primitives and token-driven spacing.
...
2. Introduce the shared actions surface component and migrate header/menu/review triggers to it.
3. Update the mobile chat controller to build task and task-list proposals from the conversation transcript.
4. Update the review UI and capability gating so only supported transforms are shown.
5. Thread the selected artifact type through accept/reject, cache updates, and header resolution.
6. Add tests for the supported menu actions, review persistence, and cache/title behavior.
