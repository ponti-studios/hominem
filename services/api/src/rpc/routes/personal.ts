...
routes
  .get('/calendar/search', authMiddleware, zValidator('query', calendarSearchQuerySchema), async (c) => {
  ...
  .get('/calendar/upcoming', authMiddleware, zValidator('query', calendarUpcomingQuerySchema), async (c) => {
    const userId = c.get('userId')!;
    const input = c.req.valid('query');
