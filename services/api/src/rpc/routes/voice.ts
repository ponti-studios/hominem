...
export const voiceRoutes = new Hono<AppContext>()
  .post('/transcribe', async (c) => {
    try {
      ...
      const language = typeof languageInput === 'string' && languageInput.trim().length > 0
        ? languageInput.trim()
        : undefined
...
export const authenticatedVoiceRoutes = new Hono<AppContext>()
  ...
  .post('/respond', async (c) => {
    try {
      ...
      const voice: Voice = (VALID_VOICES as readonly string[]).includes(rawVoice) ? rawVoice as Voice : 'alloy'
