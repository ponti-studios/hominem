...
- 2026-04-18: Executed bootstrap. Created `apps/native/` with XcodeGen project.yml, 4 schemes (Hakumi Dev, Hakumi E2E, Hakumi Preview, Hakumi), 5 build configurations, and placeholder Swift sources. `xcodebuild build -scheme "Hakumi Dev"` exits 0.
...
**Follow-up**: The `Hakumi.xcodeproj` is checked into git alongside `project.yml`. Running `xcodegen generate` in `apps/native/` regenerates it at any time.
