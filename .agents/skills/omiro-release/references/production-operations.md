...
Deliver every production change as a new TestFlight candidate, then approve and release it through App Store Connect. `pnpm build:prod`/`pnpm submit` (which also run the identity check via `scripts/verify-release-identity.mjs`) exist for ad hoc cloud use, but `just mobile release` is the standard path.
