      ...
      onError: (error, _variables, context, mutationContext) => {
        if (context && typeof context === 'object') {
          if ('previousLists' in context) {
            utils.setData(queryKeys.lists.all(), (context as any).previousLists);
          }
          if ('previousPlaces' in context) {
            utils.setData(queryKeys.places.all(), (context as any).previousPlaces);
          }
          if ('previousTrips' in context) {
            utils.setData(queryKeys.trips.all(), (context as any).previousTrips);
          }
          if ('previousPeople' in context) {
            utils.setData(queryKeys.people.list(), (context as any).previousPeople);
          }
          if ('previousInvitesSent' in context) {
            utils.setData(queryKeys.invites.sent(), (context as any).previousInvitesSent);
          }
          if ('previousInvitesReceived' in context) {
            utils.setData(queryKeys.invites.received(), (context as any).previousInvitesReceived);
          }
        }
