...
export default function App() {
  ...
  const addBehavior = () => {
    ...
    const phaseIndex = updatedBehaviors.findIndex((p) => p.phase === newBehavior.phase)
    if (phaseIndex !== -1) {
  ...
  return (
    <div className="p-6 max-w-4xl mx-auto">
      <h1 className="text-3xl font-bold text-center mb-6">90-Day Behavioral Challenge</h1>
      <div className="mb-6">
        <Input
          placeholder="Phase"
          value={newBehavior.phase}
          onChange={(e) => setNewBehavior({ ...newBehavior, phase: e.target.value })}
          className="mb-2"
        />
        <Input
          placeholder="Title"
        ...
        <Input
          ...
          onChange={(e) => setNewBehavior({ ...newBehavior, start: e.target.value })}
          className="mb-2"
        />
        <Input
          placeholder="End Date"
          value={newBehavior.end}
          onChange={(e) => setNewBehavior({ ...newBehavior, end: e.target.value })}
          className="mb-2"
        />
        <Input
          placeholder="Duration"
          value={newBehavior.duration}
          onChange={(e) => setNewBehavior({ ...newBehavior, duration: e.target.value })}
          className="mb-2"
        />
        <Button onClick={addBehavior}>Add Behavior</Button>
      </div>
      {behaviors.map((phase, index) => (
        <div key={crypto.getRandomValues(new Uint32Array(1))[0]} className="mb-6">
              ...
              >
                <Card className="p-4 shadow-md rounded-lg border border-gray-200">
                  <CardContent>
                    ...
                    <p className="text-sm text-gray-500">Duration: {behavior.duration} days</p>
                  </CardContent>
                </Card>
              </motion.div>
            ))}
