...
export function useImportTransactionsStore() {
  ...
  const progressUpdateTimeoutRef = useRef<NodeJS.Timeout | undefined>(
    undefined
  );
      ...
      progressUpdateTimeoutRef.current = setTimeout(() => {
        ...
        setStatuses((prevStatuses) => {
          const updatedStatuses = prevStatuses.length
            ? prevStatuses.map((status) => {
                const matchingJob = latestJobData.find(
                  (job) => job.fileName === status.file.name
                );
        ...
        if (completedJobs.length > 0) {
          ...
          setActiveJobIds((prev) =>
            prev.filter((id) => !completedJobIds.includes(id))
          );
        }
  ...
  const importMutation = useMutation({
      ...
      try {
        ...
        const results = await Promise.all(
          files.map(async (file) => {
            try {
              ...
              const result =
                await apiClient.postFormData<ImportRequestResponse>(
                  "/api/finance/import",
                  formData
                );
        ...
        const error =
          err instanceof Error
            ? err
            : new Error("Failed to import transactions");
        setError(error);
        throw error;
      }
    },
    ...
    onError: (err) => {
      setError(
        err instanceof Error ? err : new Error("Failed to import transactions")
      );
    },
  });
  ...
  const removeFileStatus = useCallback((fileName: string) => {
    setStatuses((prev) =>
      prev.filter((status) => status.file.name !== fileName)
    );
  }, []);
