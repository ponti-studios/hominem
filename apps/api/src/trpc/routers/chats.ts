  ...
  generate: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      try {
        ...
        ;(async () => {
          ...
          try {
            for await (const chunk of stream.textStream) {
              ...
              const charsSinceLastUpdate = accumulatedContent.length - lastUpdateLength
              ...
              if (
                ...
                while (!updateSuccess && updateRetryCount < MAX_UPDATE_RETRIES) {
                  try {
                    await messageService.updateMessage(assistantMessage.id, accumulatedContent)
                    ...
                    if (updateRetryCount < MAX_UPDATE_RETRIES) {
                      ...
                      console.error('Max retries exceeded for message update, continuing stream')
                    }
                  }
                }
              }
            ...
            await messageService.updateMessage(assistantMessage.id, finalContent)
            ...
            if (formattedToolCalls && formattedToolCalls.length > 0) {
              ...
              const currentMessages = await messageService.getChatMessages(currentChat.id, {
                limit: 100,
                orderBy: 'desc',
              })
              ...
              if (currentMessage) {
                ...
                await messageService.deleteMessage(assistantMessage.id, userId || '')
        ...
        return handleStreamError(error, 'Failed to generate streaming response')
      }
    }),
