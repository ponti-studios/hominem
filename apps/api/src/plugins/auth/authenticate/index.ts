...
async function validateEmailToken(t: string, email: string) {
  const [fetchedToken] = await db
    .select()
    .from(token)
    .where(and(eq(token.emailToken, t), gt(token.expiration, new Date().toISOString())))
    ...
    async (request, reply) => {
      try {
        ...
        if (!fetchedEmailToken.users) {
          ...
          return reply.code(401).send({ error: 'No user found matching token' })
        }

        const tokenBase = {
          isAdmin: fetchedEmailToken.users.isAdmin,
          roles: ['user', !!fetchedEmailToken.users.isAdmin && 'admin'].filter(Boolean),
          userId: fetchedEmailToken.users.id,
        }

        const accessToken = server.jwt.sign(tokenBase)

        const newUser = await userService.createOrUpdateUser(email, accessToken)
        ...
        request.log.error({ error, requestId: request.id }, 'Authentication failed')
