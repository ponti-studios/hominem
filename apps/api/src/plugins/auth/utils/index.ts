...
export const verifySession: preValidationHookHandler = async (
   ...
   */
  if (userId) {
    user = await db.select().from(users).where(eq(users.id, userId)).then(takeUniqueOrThrow)
  }
  ...
  if (!userId) {
    ...
    try {
      user = await db.select().from(users).where(eq(users.id, token.userId)).then(takeUniqueOrThrow)
