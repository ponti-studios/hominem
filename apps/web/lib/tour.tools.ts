...
export const accommodationCostCalculatorTool = tool({
	...
	async execute(details): Promise<number> {
		const { startDate, endDate, hotelCostPerNight, alternativeLodgingCost } =
			details;
...
export const get_performance_location_suggestions = tool({
	description:
		"Search for regions the user could perform if they do not provide a location",
	...
	execute: async ({ query }) => {
		if (query.eventId) {
			return PERFORMANCES.filter(
				(performance) => performance.id === query.eventId,
			);
		}
...
export const get_performance_finances_by_id = tool({
	...
	execute: async ({ eventId }) => {
		return PERFORMANCE_FINANCES.filter(
			(finances) => finances.eventId === eventId,
		);
	},
});
