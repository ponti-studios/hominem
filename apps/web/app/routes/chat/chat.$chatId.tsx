...
export default function ChatPage({ params }: Route.ComponentProps) {
  ...
  const classifyMutation = useRpcMutation(
    ({ chats }, vars: { targetType: ArtifactType }) => chats.classify({ chatId, targetType: vars.targetType }),
  );

  const acceptMutation = useRpcMutation(
    ({ review }, vars: { reviewItemId: string }) => review.accept({ reviewItemId: vars.reviewItemId }),
  );

  const rejectMutation = useRpcMutation(
    ({ review }, vars: { reviewItemId: string }) => review.reject({ reviewItemId: vars.reviewItemId }),
  );
