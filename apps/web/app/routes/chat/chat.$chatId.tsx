...
export default function ChatPage({ params }: Route.ComponentProps) {
  ...
  useEffect(() => {
    if (seedNote) {
      setSelectedNotes((current) =>
        current.some((note) => note.id === seedNote.id)
          ? current
          : [...current, { id: seedNote.id, title: seedNote.title, excerpt: seedNote.excerpt }],
      );
      setDraft((current) => {
  ...
  async function handleSend() {
    if (!draft.trim() && attachedFiles.length === 0 && selectedNotes.length === 0) {
      return;
    }
          ...
          <div className="mt-3 flex flex-wrap gap-2">
            ...
            {selectedNotes.map((note) => (
              <button
                ...
                onClick={() => {
                  setSelectedNotes((current) => current.filter((item) => item.id !== note.id));
                }}
              >
                {note.title || 'Untitled note'} x
              </button>
            ))}
          </div>
        </div>
                ...
                {suggestions.map((note) => (
                  <button
                    ...
                    onClick={() => {
                      ...
                      setSelectedNotes((current) => [...current, note]);
                      setDraft((current) => current.replace(/#[a-z0-9-]*$/i, `#${slug} `));
