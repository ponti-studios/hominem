...
const mocks = vi.hoisted(() => {
  ...
  class MockUppy {
    private listeners = new Map<string, Set<UploadSuccessHandler | ((progress: number) => void) | ((file: MockUploadFile | null, error: Error) => void)>>()
    ...
    use(_plugin: unknown, options: { getUploadParameters?: (file: MockUploadFile) => Promise<unknown> }) {
      this.awsPluginOptions = options
      return this
    }

    on(event: string, handler: UploadSuccessHandler | ((progress: number) => void) | ((file: MockUploadFile | null, error: Error) => void)) {
    ...
    off(event: string, handler: UploadSuccessHandler | ((progress: number) => void) | ((file: MockUploadFile | null, error: Error) => void)) {
    ...
    setFileMeta(fileId: string, meta: Record<string, string | number | boolean | null | undefined>) {
