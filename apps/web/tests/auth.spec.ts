...
test('notes auth falls back from passkey entry to email otp successfully', async ({
  page,
  context,
}) => {
...
test('notes email + otp rejects invalid verification code', async ({ page, context }) => {
  ...
  await expect(page.getByText('Invalid OTP')).toBeVisible({ timeout: 5_000 });
});

test('notes authenticated surfaces expose passkey enrollment controls', async ({
  page,
  context,
}) => {
  await context.clearCookies();
