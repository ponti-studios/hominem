...
| F007 | Dependency debt | `apps/omiro/package.json:64` | Low | S | `expo-updates` has zero JS references; `app.config.ts` sets `runtimeVersion` but has no `updates` block | Confirm with the team whether `expo-updates` is required as a native dependency for EAS Update even without a JS import — see open questions |
