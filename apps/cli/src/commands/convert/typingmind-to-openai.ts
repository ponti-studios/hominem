...
export const command = new Command('typingmind-to-openai')
  ...
  .action(async (options) => {
    try {
      ...
      console.log(`Successfully parsed Typing Mind data with ${typingMindData.data.chats.length} conversations`);
      
      // Convert data
      const openAIData = convertTypingMindToOpenAI(typingMindData);
      console.log(`Converted ${openAIData.length} conversations to OpenAI format`);
      
      // Save output
      const outputPath = path.resolve(options.output);
      writeFileSync(outputPath, JSON.stringify(openAIData, null, 2));
      console.log(`Saved OpenAI export to ${outputPath}`);
    } catch (error) {
