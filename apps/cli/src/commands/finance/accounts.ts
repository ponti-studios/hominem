...
command.name('accounts').description('Manage financial accounts')
  ...
  .action(async (options) => {
    ...
    try {
      const accountsList = await db
        .select()
        .from(accounts)
        .where(options.activeOnly ? eq(accounts.isActive, true) : undefined)
      ...
      for (const account of accountsList) {
        console.log(
          `${account.id} | ${account.name} | ${account.mask || '-'} | ${account.type || '-'} | ${account.institution || '-'} | ${account.isActive ? 'Yes' : 'No'}`
        )
      }
    } catch (error) {
  ...
  .action(async (id, options) => {
    ...
    try {
      ...
      const account = await db
        .select()
        .from(accounts)
        .where(eq(accounts.id, Number.parseInt(id)))
        .limit(1)
