...
command
  ...
  .action(async () => {
    ...
    try {
      ...
      const duplicateAccountsQuery = await db
        .select({
      ...
      const duplicateNamesQuery = await db
        .select({
      ...
      if (duplicateAccountsQuery.length > 0) {
        logger.info(
          `Found ${duplicateAccountsQuery.length} transaction-account pairs with duplicates`
        )
        ...
        for (const duplicate of duplicateAccountsQuery) {
          ...
          await db
            ...
            .where(sql`transaction_id = ${transactionId} AND account_name = ${accountName}`)
            .limit(duplicatesToRemove)

          totalDuplicatesRemoved += duplicatesToRemove
          logger.info(
            `Removed ${duplicatesToRemove} duplicate records for transaction ${transactionId}, account ${accountName}`
          )
        }
      }
      ...
      if (duplicateNamesQuery.length > 0) {
        ...
        for (const duplicate of duplicateNamesQuery) {
          ...
          await db
            .delete(transactionNames)
            .where(sql`transaction_id = ${transactionId} AND name = ${name}`)
            .limit(duplicatesToRemove)

          totalDuplicatesRemoved += duplicatesToRemove
          logger.info(
            `Removed ${duplicatesToRemove} duplicate records for transaction ${transactionId}, name ${name}`
          )
        }
      }
      ...
      if (orphanedAccounts.length > 0) {
        ...
        for (const orphan of orphanedAccounts) {
          await db
            .delete(transactionAccounts)
            .where(sql`transaction_id = ${orphan.id} AND account_name = ${orphan.accountName}`)
        }
      ...
      if (orphanedNames.length > 0) {
        ...
        for (const orphan of orphanedNames) {
          await db
            .delete(transactionNames)
            .where(sql`transaction_id = ${orphan.id} AND name = ${orphan.name}`)
        }
      ...
      spinner.succeed(
        `Cleanup completed: Removed ${totalDuplicatesRemoved} duplicate account records`
      )
      ...
      logger.error(error)
      console.error(
        `Error cleaning up transactions: ${error instanceof Error ? error.message : error}`
      )
      process.exit(1)
    }
  })
