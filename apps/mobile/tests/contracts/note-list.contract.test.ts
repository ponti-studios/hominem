...
describe('notes list contract', () => {
  ...
  it('supports a reverse chronological feed view without changing deterministic traversal', () => {
    const dataset = buildNoteItems(120).reverse()
    const traversal = simulateNoteScrollTraversal(dataset, 20, 10)

    expect(traversal.at(0)).toBe('note-0119')
    expect(traversal.at(-1)).toBe('note-0000')
  })
