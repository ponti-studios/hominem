...
export const useVoidExit = () => {
  ...
  const triggerExit = React.useCallback(
    (onComplete?: () => void) => {
      opacity.value = withTiming(0, { duration: VOID_MOTION_EXIT, easing: VOID_EASING_EXIT }, (finished) => {
        if (finished && onComplete) onComplete()
      })
      translateY.value = withTiming(VOID_EXIT_TRANSLATE_Y, { duration: VOID_MOTION_EXIT, easing: VOID_EASING_EXIT })
    },
    [opacity, translateY]
  )
