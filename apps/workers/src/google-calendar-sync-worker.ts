...
const processGoogleCalendarSyncJob = async (
  job: Job<GoogleCalendarSyncPayload>
) => {
  logger.info(
    `Processing Google Calendar sync job ${job.id} for user ${job.data.userId}`
  );
  ...
  try {
    ...
    // Sync events
    const result = await googleService.syncGoogleCalendarEvents(
      calendarId,
      timeMin
    );
...
const gracefulShutdown = async (signal: string) => {
  ...
  logger.info(
    `${signal} received, shutting down Google Calendar worker gracefully...`
  );
