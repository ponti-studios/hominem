...
export async function getJobApplicationMetrics(
  ...
  const interviewsScheduled = applications.filter((application) => application.firstInterviewDate).length
  const offersReceived = applications.filter(
    (application) => application.status === 'OFFER' || application.status === 'ACCEPTED'
  ).length
  const offersAccepted = applications.filter((application) => application.status === 'ACCEPTED').length
  ...
  return {
    averageTimeToResponse:
      timeToResponseApps.length > 0
        ? timeToResponseApps.reduce((sum, application) => sum + (application.timeToResponse || 0), 0) /
    ...
    averageTimeToDecision:
      timeToDecisionApps.length > 0
        ? timeToDecisionApps.reduce((sum, application) => sum + (application.timeToDecision || 0), 0) /
    ...
    averageNegotiationIncrease:
      negotiatedApps.length > 0
        ? negotiatedApps.reduce((sum, application) => {
            ...
            return sum + calculatePercentageChange(application.salaryOffered, application.salaryNegotiated)
...
export async function getJobApplicationFunnel(userId: string): Promise<JobApplicationFunnel[]> {
  ...
  const funnel = [
    ...
    { stage: 'Response', count: applications.filter((application) => application.responseDate).length },
    {
      stage: 'Phone Screen',
      count: applications.filter(
        (application) =>
          application.status === 'PHONE_SCREEN' ||
          (application.interviewDates && Array.isArray(application.interviewDates) && application.interviewDates.length > 0)
      ).length,
    },
    { stage: 'Interview', count: applications.filter((application) => application.firstInterviewDate).length },
    { stage: 'Final Round', count: applications.filter((application) => application.status === 'FINAL_INTERVIEW').length },
    {
      stage: 'Offer',
      count: applications.filter(
        (application) => application.status === 'OFFER' || application.status === 'ACCEPTED'
    ...
    { stage: 'Accepted', count: applications.filter((application) => application.status === 'ACCEPTED').length },
  ]
...
export async function getApplicationsByTimeframe(
  ...
  return applications.filter(
    (application) => application.applicationDate && new Date(application.applicationDate) >= cutoffDate
  )
}
...
export async function getAllApplicationsWithCompany(
  ...
  let applications = extractJobApplications(await getUserJobApplications(userId)) as ApplicationWithCompany[]
  ...
  if (filter?.statuses?.length) {
    applications = applications.filter((application) => filter.statuses?.includes(application.status))
  }
  ...
  if (filter?.startDate) {
    applications = applications.filter(
      (application) => application.applicationDate && new Date(application.applicationDate) >= filter.startDate!
    )
  }

  if (filter?.endDate) {
    applications = applications.filter(
      (application) => application.applicationDate && new Date(application.applicationDate) <= filter.endDate!
    )
  }
