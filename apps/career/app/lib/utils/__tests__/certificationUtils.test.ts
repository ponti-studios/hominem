...
import { formatCertificationStatus, getCertificationStatusClasses } from '../certificationUtils';

describe('certificationUtils', () => {
  it('formats certification statuses for display', () => {
    expect(formatCertificationStatus('pending_renewal')).toBe('Pending Renewal');
  ...
  it('returns certification status classes by status', () => {
    ...
    expect(getCertificationStatusClasses('archived')).toBe('bg-muted text-foreground');
  });
});
