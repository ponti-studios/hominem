...
export function DesktopAuthProvider({ children }: { children: ReactNode }) {
  ...
  const handleRequestOtp = useCallback(async (nextEmail: string) => {
  ...
  }), [apiBaseUrl, email, handlePasskeySignIn, handleRequestOtp, handleSignOut, handleVerifyOtp, session, state])
