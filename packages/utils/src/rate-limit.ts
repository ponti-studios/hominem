...
type RateLimitFactoryOptions = {
  ...
  getCustomError: (req: any, key: string) => any;
  maxHits: number;
  onRateLimit: (req: any, key: string) => void;
  segment: string;
  windowLength: number;
};

type Server = {
  decorate: (type: string, name: string, method: any) => void;
};

// Helper to rate limit a route by a key (like ip or user id)
const rateLimitDecoratorFactory = (opts: RateLimitFactoryOptions) => {
  return (server: Server) => {
    ...
    async function rateLimit(req: Request, key: string) {
