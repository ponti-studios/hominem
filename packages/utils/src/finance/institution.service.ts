...
/**
 * Get all institution connections for a user
 * This aggregates Plaid Items into institution-centric data
 */
export async function getUserInstitutionConnections(userId: string): Promise<InstitutionConnection[]> {
  try {
    // Get all institutions the user has connections to (both Plaid and manual)
    const connections = await db
      ...
      .leftJoin(plaidItems, and(
        eq(financialInstitutions.id, plaidItems.institutionId),
        eq(plaidItems.userId, userId)
      ))
      .leftJoin(financeAccounts, and(
 ...
 */
export async function getInstitutionAccounts(userId: string, institutionId: string): Promise<InstitutionAccount[]> {
  try {
    const accounts = await db
      .select({
        id: financeAccounts.id,
      ...
      .where(and(
        eq(financeAccounts.userId, userId),
        eq(financeAccounts.institutionId, institutionId)
      ))
