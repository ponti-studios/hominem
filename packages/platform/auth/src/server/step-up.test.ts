...
describe('step-up authentication', () => {
  ...
  describe('isFreshPasskeyAuth', () => {
    ...
    it('returns true for recent passkey auth', () => {
      ...
      expect(isFreshPasskeyAuth({ amr: ['passkey'], authTime: authTimeSeconds, nowMs: now })).toBe(
        true,
      );
    });
    ...
    it('returns false when amr does not include passkey', () => {
      ...
      expect(isFreshPasskeyAuth({ amr: ['password'], authTime: authTimeSeconds, nowMs: now })).toBe(
        false,
      );
    });
