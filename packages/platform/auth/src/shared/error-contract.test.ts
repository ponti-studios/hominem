...
describe('readAuthErrorMessage', () => {
  ...
  it('maps known error codes to messages', () => {
    expect(readAuthErrorMessage(new URLSearchParams('error=access_denied'))).toBe(
      'Access was denied.',
    );
    expect(readAuthErrorMessage(new URLSearchParams('error=server_error'))).toBe(
      'A server error occurred. Please try again.',
    );
    expect(readAuthErrorMessage(new URLSearchParams('error=temporarily_unavailable'))).toBe(
