...
import { FINANCE_TRANSACTION_ENTITY_TYPE, financeTransactionQueryContractSchema } from './contracts'
...
export function calculateSavingsGoal(
  input: z.infer<typeof calculateSavingsGoalInputSchema>,
): {
  monthsToGoal: number
...
export function calculateLoanDetails(
  input: z.infer<typeof calculateLoanDetailsInputSchema>,
): {
...
export async function exportFinanceData(userId: string): Promise<{
  ...
  budgetGoals: Array<{ id: string; categoryId: string | null; targetAmount: number; targetPeriod: string }>
  ...
  let budgetGoals: Array<{ id: string; categoryId: string | null; targetAmount: number; targetPeriod: string }> = []
  if (await tableExists('budget_goals')) {
    const budgetGoalsResult = await db.execute(sql`
...
export async function getAccountById(accountId: string, userId?: string): Promise<FinanceAccount | null> {
  if (userId) {
    const result = await db.execute(sql`
      select id, user_id, name, account_type, balance, data
      from finance_accounts
...
export async function updateAccount(input: Partial<FinanceAccount> & { id: string }): Promise<FinanceAccount | null> {
...
export async function listAccountsWithRecentTransactions(userId: string): Promise<FinanceAccount[]> {
  return listAccounts(userId)
}

export async function getAccountWithPlaidInfo(accountId: string, userId: string): Promise<FinanceAccount | null> {
...
export async function getTransactionTagAnalysis(_userId: string): Promise<Array<{ tag: string; total: number }>> {
...
export async function getBudgetCategoryById(id: string, userId: string): Promise<FinanceCategory | null> {
  const result = await db.execute(sql`
    select id, owner_id, name, color
    from tags
    where id = ${id}
...
export async function getBudgetTrackingData(userId: string): Promise<{ totalBudget: number; totalSpent: number }> {
...
export function calculateRunway(input: z.infer<typeof runwayCalculationSchema>): { months: number } {
...
export async function getTagBreakdown(_userId: string): Promise<Array<{ tag: string; total: number }>> {
  const result = await db.execute(sql`
    select
      tg.name as tag,
...
export async function getTopMerchants(_userId: string): Promise<Array<{ merchant: string; total: number }>> {
  const result = await db.execute(sql`
    select
...
export async function getTagBreakdownByContract(
  input: Omit<Partial<FinanceTransactionQueryContract>, 'userId'> & { userId: string; limit?: number },
...
export async function getSpendingTimeSeriesByContract(input: {
  ...
  const sortedDates = [...bucketData.keys()].sort((a, b) => a.localeCompare(b)).slice(-normalizedLimit)
  ...
  return {
    ...
    stats: {
      ...
      averageIncome: data.length === 0 ? 0 : data.reduce((sum, item) => sum + item.income, 0) / data.length,
      averageExpenses: data.length === 0 ? 0 : data.reduce((sum, item) => sum + item.expenses, 0) / data.length,
      count: data.length,
...
export async function queryTransactionsByContract(
  ...
  const tagNames = parsed.tagNames ?? []
  if (tagIds.length > 0 && tagNames.length > 0) {
    const filters = [...baseFilters, sql`(ti.tag_id in (${sqlValueList(tagIds)}) or tg.name in (${sqlValueList(tagNames)}))`]
    const result = await db.execute(sql`
...
export async function getPlaidItemByUserAndItemId(_userId: string, _itemId: string): Promise<PlaidItem | null> {
  const result = await db.execute(sql`
    select id, user_id, item_id, institution_id, cursor, access_token, status, last_synced_at
    from plaid_items
    where user_id = ${_userId}
...
export async function upsertAccount(input: Partial<FinanceAccount> & { userId: string }): Promise<FinanceAccount> {
  if (!input.name) {
...
export async function getAccountByPlaidId(plaidAccountId: string, userId?: string): Promise<FinanceAccount | null> {
  if (userId) {
    const result = await db.execute(sql`
      select id, user_id, name, account_type, balance, data
      from finance_accounts
...
export async function processTransactionsFromCSVBuffer(
  _input: { userId: string; accountId: string; csvBuffer: ArrayBuffer | Buffer },
  ...
  const nextAmount =
    updates.amount === undefined
      ...
      : typeof updates.amount === 'string'
        ...
        : updates.amount
  const nextDescription = updates.description === undefined ? existing.description : updates.description
  const nextDate =
