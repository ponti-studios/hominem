...
export function TopTags({ dateFrom, dateTo, selectedAccount }: TopTagsProps) {
  const { data: categoryBreakdown, isLoading, error } = useTagBreakdown({
    from: dateFrom,
    to: dateTo,
  ...
  return (
    <Card>
      ...
      <CardContent>
        ...
        ) : Array.isArray(categoryBreakdown?.breakdown) && categoryBreakdown.breakdown.length > 0 ? (
          <div className="space-y-3">
            {categoryBreakdown.breakdown.map((cat: { tag: string; amount: number; transactionCount: number }) => {
