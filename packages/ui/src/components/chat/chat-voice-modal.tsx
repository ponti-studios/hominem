    ...
    >
      <div ref={dialogRef} className="w-full max-w-md space-y-4 border bg-background p-6">
        <Inline justify="between">
          ...
          <Button ref={closeButtonRef} variant="ghost" size="sm" onClick={onClose} aria-label="Close voice input">
            <X className="size-4" />
          </Button>
        </Inline>
        ...
        <Inline gap="sm">
          <SpeechInput
            ...
            onPermissionDenied={() => {
              setPermissionError('Microphone access blocked. Please allow microphone access and try again.')
            }}
            {...(onRecordingStateChange ? { onRecordingStateChange } : {})}
          />
        </Inline>
        {mergedErrorMessage ? <p className="text-sm text-destructive" role="alert">{mergedErrorMessage}</p> : null}
      </div>
    </div>
  )
}
