'use client';

import { useSafeAuth } from '@hominem/auth';
import { Button } from '@hominem/ui/button';
import {
  DropdownMenu,
  DropdownMenuContent,
  DropdownMenuItem,
  DropdownMenuTrigger,
} from '@hominem/ui/dropdown';
import { LogOut, MenuIcon, Settings, type LucideIcon } from 'lucide-react';
import { useCallback } from 'react';
import { Link, useNavigate } from 'react-router';

export interface NavItem {
  title: string;
  url: string;
  icon?: LucideIcon;
}

export interface HeaderProps {
  brandName: string;
  brandIcon?: React.ReactNode;
  navItems?: NavItem[];
}

function NavigationMenu({ navItems = [] }: { navItems?: NavItem[] }) {
  const navigate = useNavigate();
  const authContext = useSafeAuth();
  if (!authContext) return null;
  const { logout } = authContext;
  const onLogoutClick = useCallback(async () => {
    await logout();
    navigate('/');
  }, [logout, navigate]);

  return (
    <DropdownMenu modal={false}>
      <DropdownMenuTrigger asChild>
        <Button
          variant="ghost"
          size="icon"
          className="px-2 text-primary focus-visible:ring-2 ring-primary"
        >
          <MenuIcon className="size-5" />
        </Button>
      </DropdownMenuTrigger>
      <DropdownMenuContent
        className="w-56"
        align="end"
        sideOffset={8}
        avoidCollisions={true}
        side="bottom"
      >
        {navItems?.map((item) => (
          <DropdownMenuItem key={item.title} asChild className="py-2">
            <Link to={item.url} prefetch="intent" className="flex items-center space-x-2">
              {item.icon && <item.icon className="size-4" />}
              <span>{item.title}</span>
            </Link>
          </DropdownMenuItem>
        ))}
        <DropdownMenuItem asChild className="py-2">
          <Link to="/account" prefetch="intent" className="flex items-center space-x-2">
            <Settings className="size-4" />
            <span>Account</span>
          </Link>
        </DropdownMenuItem>
        <DropdownMenuItem className="py-2 flex items-center space-x-2" onClick={onLogoutClick}>
          <LogOut className="size-4" />
          <span>Log out</span>
        </DropdownMenuItem>
      </DropdownMenuContent>
    </DropdownMenu>
  );
}

function SignInButton() {
  const authContext = useSafeAuth();
  if (!authContext) return null;

  const onSignInClick = useCallback(() => {
    window.location.href = '/auth/email';
  }, []);

  return (
    <Button onClick={onSignInClick} className="flex">
      Sign In with Email
    </Button>
  );
}

export function Header({ brandName, brandIcon, navItems }: HeaderProps) {
  const authContext = useSafeAuth();

  if (!authContext) {
    return (
      <header
        className="fixed top-0 left-0 z-50 w-full bg-background/10 backdrop-blur-md border-b border-border/50"
        style={{ paddingRight: 'var(--removed-body-scroll-bar-size, 0px)' }}
      >
        <div className="flex px-2 py-4 md:px-8 items-center justify-between">
          <Link to="/" prefetch="intent" className="flex items-center space-x-2 text-primary">
            {brandIcon}
            <span className="heading-2">{brandName}</span>
          </Link>
        </div>
      </header>
    );
  }

  const { isAuthenticated, isLoading } = authContext;

  return (
    <header
      className="fixed top-0 left-0 z-50 w-full bg-background/10 backdrop-blur-md border-b border-border/50"
      style={{ paddingRight: 'var(--removed-body-scroll-bar-size, 0px)' }}
    >
      <div className="flex px-2 py-4 md:px-8 items-center justify-between">
        <Link to="/" prefetch="intent" className="flex items-center space-x-2 text-primary">
          {brandIcon}
          <span className="heading-2">{brandName}</span>
        </Link>
        {!isLoading && (
          <div className="flex items-center space-x-2">
            {isAuthenticated ? <NavigationMenu navItems={navItems ?? []} /> : <SignInButton />}
          </div>
        )}
      </div>
    </header>
  );
}
