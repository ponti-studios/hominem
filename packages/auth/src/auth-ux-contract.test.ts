...
describe('AppAuthConfig — per-app destination policy', () => {
  ...
  describe('CHAT_AUTH_CONFIG', () => {
    it('has a non-empty appName', () => {
      expect(CHAT_AUTH_CONFIG.appName.length).toBeGreaterThan(0)
    })

    it('uses the Hakumi brand name', () => {
      expect(CHAT_AUTH_CONFIG.appName).toBe('Hakumi')
    })

    it('has a valid mobile post-auth destination', () => {
      expect(CHAT_AUTH_CONFIG.defaultPostAuthDestination.length).toBeGreaterThan(0)
    })

    it('uses canonical AUTH_COPY', () => {
      expect(CHAT_AUTH_CONFIG.copy).toBe(AUTH_COPY)
    })
  })

  describe('cross-platform copy parity', () => {
    it('both apps use the same AUTH_COPY reference', () => {
      expect(NOTES_AUTH_CONFIG.copy).toBe(CHAT_AUTH_CONFIG.copy)
    })

    it('both apps share the same email entry copy', () => {
      expect(NOTES_AUTH_CONFIG.copy.emailEntry).toEqual(CHAT_AUTH_CONFIG.copy.emailEntry)
    })

    it('both apps share the same OTP verification copy', () => {
      expect(NOTES_AUTH_CONFIG.copy.otpVerification).toEqual(CHAT_AUTH_CONFIG.copy.otpVerification)
    })

    it('both apps share the same passkey copy', () => {
      expect(NOTES_AUTH_CONFIG.copy.passkey).toEqual(CHAT_AUTH_CONFIG.copy.passkey)
    })
    ...
    it('post-auth destinations are different per platform', () => {
      ...
      expect(NOTES_AUTH_CONFIG.defaultPostAuthDestination).not.toBe(
        CHAT_AUTH_CONFIG.defaultPostAuthDestination,
      )
    })
  })
})
