...
export function getServerAuthConfig(): AuthConfig {
  ...
  if ((!supabaseUrl || !supabaseAnonKey) && typeof process.env !== 'undefined') {
...
/**
 * Simplified version that reads environment variables automatically
 * Use this in app loaders instead of the full createSupabaseServerClient
 */
export function createSupabaseClient(request: Request) {
  const config = getServerAuthConfig()
  return createSupabaseServerClient(request, config)
}
...
export async function getAuthState(
  request: Request
): Promise<{ user: SupabaseAuthUser | null; isAuthenticated: boolean; headers: Headers }> {
