...
function toGoalProgress(goal: { currentValue: number | null; targetValue: number | null }): GoalStats {
...
export async function getGoalsByUser(
  ...
  let goals = (await getEvents()).filter((event) => event.userId === userId && event.type === 'Goal')
...
export async function getGoalById(goalId: string, userId?: string): Promise<EventWithTagsAndPeople | null> {
  const goal = await getEventById(goalId)
