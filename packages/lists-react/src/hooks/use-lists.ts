...
export const useLists = (
  ...
  useRpcQuery(async ({ lists }) => lists.getAll({} satisfies ListGetAllInput), {
  ...
  return useRpcMutation<ListUpdateOutput, ListUpdateInput>(
    async ({ lists }, variables) => lists.update(variables),
    {
      ...
      onMutate: async (variables) => {
        ...
        const previousLists = queryClient.getQueryData<ListGetAllOutput>(queryKeys.lists.all());
        const previousList = queryClient.getQueryData<ListGetByIdOutput>(
          queryKeys.lists.get(variables.id),
        );
      ...
      onError: (error, variables, context, mutationContext) => {
        ...
        if (previousList) {
          queryClient.setQueryData<ListGetByIdOutput>(
            queryKeys.lists.get(variables.id),
            previousList,
          );
        }
