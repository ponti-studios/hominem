...
export class NotesService {
  async create(input: NoteInsert): Promise<Note> {
    ...
    const noteData: NoteInsert = {
      ...
      taskMetadata:
        input.taskMetadata === null ? undefined : input.taskMetadata,
  ...
  async list(
    ...
    if (ftsQuery) {
      conditions.push(
        sql`${tsvector_sql} @@ websearch_to_tsquery('english', ${ftsQuery})`
      );
    }
    ...
    if (filters?.tags && filters.tags.length > 0) {
      for (const tag of filters.tags) {
        conditions.push(
          sql`${notes.tags}::jsonb @> ${JSON.stringify([
            { value: tag },
          ])}::jsonb`
        );
      }
  ...
  async update(input: UpdateNoteInput): Promise<Note> {
    ...
    if (validatedInput.taskMetadata !== undefined)
      updateData.taskMetadata =
        validatedInput.taskMetadata === null
          ? undefined
          : validatedInput.taskMetadata;
    if (validatedInput.analysis !== undefined)
      updateData.analysis =
        validatedInput.analysis === null ? undefined : validatedInput.analysis;
    ...
    const [item] = await db
      ...
      .where(
        and(
          eq(notes.id, validatedInput.id),
          eq(notes.userId, validatedInput.userId)
        )
      )
      .returning();
    if (!item) {
