...
import { db } from '../db'
import { contacts, events, eventsTags, eventsUsers, type EventTypeEnum, tags } from '../db/schema'
import {
  getPeopleForLifeEvent,
  getPeopleForLifeEvents,
  replacePeopleForLifeEvent,
} from './people.service'
import {
  addTagsToLifeEvent,
  getTagsForLifeEvent,
  getTagsForLifeEvents,
  removeTagsFromLifeEvent,
  syncTagsForLifeEvent,
} from './tags.service'
...
export async function createLifeEvent(event: LifeEventInput) {
  ...
  if (event.people && event.people.length > 0) {
    await replacePeopleForLifeEvent(result.id, event.people)
  }

  if (event.tags && event.tags.length > 0) {
    await addTagsToLifeEvent(result.id, event.tags)
  }

  const [people, tagsList] = await Promise.all([
    getPeopleForLifeEvent(result.id),
    getTagsForLifeEvent(result.id),
  ])
...
export async function updateLifeEvent(id: string, event: UpdateLifeEventInput) {
  ...
  if (event.people !== undefined) {
    await replacePeopleForLifeEvent(id, event.people)
  }
  ...
  const [people, tagsList] = await Promise.all([getPeopleForLifeEvent(id), getTagsForLifeEvent(id)])
